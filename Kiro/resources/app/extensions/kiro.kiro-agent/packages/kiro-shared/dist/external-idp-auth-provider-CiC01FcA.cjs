...
const client__namespace = /* @__PURE__ */ _interopNamespaceDefault(client);
...
class AuthCallbackHandler {
  ...
  initialize(onUri) {
    if (this.disposable) {
      errors.logger.debug("AuthCallbackHandler: Already initialized");
      return;
    }
    this.disposable = onUri((uri) => {
      this.handleUri(uri);
    });
    errors.logger.info("AuthCallbackHandler: Initialized");
  }
  /**
  ...
  waitForCallback(state) {
    return new Promise((resolve, reject) => {
      ...
      const warningTimeoutId = setTimeout(() => {
        errors.logger.warn("AuthCallbackHandler: Authentication is taking a long time for state=%s", state);
      ...
      errors.logger.debug("AuthCallbackHandler: Waiting for callback with state=%s", state);
    });
  }
  /**
  ...
  cancelCallback(state) {
    ...
    if (pending) {
      this.cleanupPendingCallback(state);
      pending.reject(new ssoOidcClient.CanceledError("user cancellation"));
      errors.logger.debug("AuthCallbackHandler: Canceled callback for state=%s", state);
    }
  }
  /**
  ...
  handleUri(uri) {
    ...
    if (uri.authority !== "kiro.oauth" || uri.path !== "/callback") {
      errors.logger.debug("AuthCallbackHandler: Ignoring URI - authority=%s, path=%s", uri.authority, uri.path);
      return;
    }
    errors.logger.info("AuthCallbackHandler: Processing auth callback URI");
      ...
      if (state2) {
        ...
        if (pending2) {
          ...
          if (error === "access_denied") {
            pending2.reject(new ssoOidcClient.AuthProviderDeniedAccess());
          } else {
            pending2.reject(new ssoOidcClient.AuthProviderFailure(errorDescription || error));
          }
    ...
    if (!state) {
      errors.logger.error("AuthCallbackHandler: Missing state parameter");
    ...
    const pending = this.pendingCallbacks.get(state);
    if (!pending) {
      errors.logger.warn("AuthCallbackHandler: No pending callback for state=%s", state);
    ...
    if (!code) {
      this.cleanupPendingCallback(state);
      pending.reject(new ssoOidcClient.MissingCodeError());
    ...
    errors.logger.info("AuthCallbackHandler: Successfully received authorization code");
    void vscode__namespace.env.openExternal(vscode__namespace.Uri.parse(authConfig.getAuthSuccessPageUrl()));
  }
  ...
  showErrorNotification(message) {
    void vscode__namespace.window.showErrorMessage(`Authentication failed: ${message}`);
  }
  /**
  ...
  dispose() {
    for (const [_state, pending] of this.pendingCallbacks) {
      clearTimeout(pending.timeoutId);
      clearTimeout(pending.warningTimeoutId);
      pending.reject(new ssoOidcClient.CanceledError("handler disposed"));
    }
    this.pendingCallbacks.clear();
    this.disposable?.dispose();
    this.disposable = void 0;
    errors.logger.debug("AuthCallbackHandler: Disposed");
  }
}
...
class AuthSSOServer {
  constructor(state) {
    ...
    this.server = http.createServer((request, response) => {
      ...
      switch (url.pathname) {
        ...
        default: {
          errors.logger.info("Unexpected invocation of AuthSSOServer. Path: %s", url.pathname);
        }
      }
    });
   ...
   */
  static init(state = "") {
    return telemetry_index.withSpan(telemetry_definitions_index.TelemetryNamespace.Auth, "auth-server.init", async () => {
      const lastInstance = AuthSSOServer.#lastInstance;
      if (lastInstance !== void 0 && !lastInstance.closed) {
        try {
          await lastInstance.close();
        } catch (error) {
          errors.logger.error("Failed to close already existing auth server in AuthSSOServer.init(): %s", error);
        }
      }
      errors.logger.debug("AuthSSOServer: Initialized new auth server.");
  ...
  /** Starts the server */
  start(ports) {
    return telemetry_index.withSpan(telemetry_definitions_index.TelemetryNamespace.Auth, "auth-server.start", async (span) => {
      let portIndex = 0;
  ...
  /** Closes the server */
  close() {
    return telemetry_index.withSpan(telemetry_definitions_index.TelemetryNamespace.Auth, "auth-server.close", async (span) => {
      try {
        span.setAttribute("redirectUri", this.redirectUri);
      } catch (_e) {
      ...
      return new Promise((resolve, reject) => {
        ...
        errors.logger.debug("AuthSSOServer: Attempting to close server.");
        ...
        this.server.close((error) => {
          ...
          this._closed = true;
          errors.logger.debug("AuthSSOServer: Server closed successfully.");
          resolve();
        });
      });
    });
  ...
  redirectToSuccess(response) {
    response.writeHead(302, { Location: authConfig.getAuthSuccessPageUrl() });
    response.end();
  }
  redirectToError(response, errorMessage) {
    response.writeHead(302, { Location: authConfig.getAuthErrorPageUrl(errorMessage) });
    response.end();
  }
  ...
  cancelCurrentFlow() {
    errors.logger.debug("AuthSSOServer: Canceling current login flow");
    this.deferred?.reject(new ssoOidcClient.CanceledError("user cancellation"));
  }
  /** Waits for authorization to complete */
  async waitForAuthorization() {
    const warningTimeout = setTimeout(() => {
      errors.logger.warn("AuthSSOServer: Authentication is taking a long time");
...
async function selectOrPromptProfile(profiles) {
  if (profiles.length === 0) {
    errors.logger.warn("No profiles available to select");
    return;
  }
  if (profiles.length === 1) {
    await vscode__namespace.commands.executeCommand("kiro.profiles.selectProfile", profiles[0]);
    return;
  }
  await vscode__namespace.commands.executeCommand("kiro.profiles.showProfileSelector", { isDismissible: true });
}
...
class ClientRegistrationStorage {
  ...
  writeClientRegistration(clientIdHash, registration) {
    ...
    try {
      fs__namespace.writeFileSync(filePath, JSON.stringify(registration, void 0, 2), { mode: 384 });
...
class IDCAuthProvider {
  ...
  async registerClient(startUrl, region, hasUserProvidedInput = false) {
    return telemetry_index.withSpan(telemetry_definitions_index.TelemetryNamespace.Auth, "idc-provider.register", async () => {
  ...
  async authenticate(options) {
    return telemetry_index.withSpan(telemetry_definitions_index.TelemetryNamespace.Auth, "idc-provider.authenticate", async (span) => {
      ...
      try {
        ...
        const trustedError = new errors.TrustedError(
        ...
        errors.logger.error("AuthServer start failed:", trustedError);
        throw trustedError;
      }
  ...
  async refreshToken(token) {
    return telemetry_index.withSpan(telemetry_definitions_index.TelemetryNamespace.Auth, "idc-provider.refreshToken", async (span) => {
      ...
      try {
        ...
        errors.logger.error("Error refreshing token:", error);
        throw error;
      }
    });
  }
  ...
  logout(token) {
    try {
      ...
      errors.logger.error("Error deleting client registration during logout:", error);
    }
  ...
  async deleteAccount(token) {
    return telemetry_index.withSpan(telemetry_definitions_index.TelemetryNamespace.Auth, "idc-provider.deleteAccount", async (span) => {
      span.setAttribute("authProvider", token.provider);
      if (token.authMethod !== "IdC") {
        throw new ssoOidcClient.UnexpectedIssueError("IdC auth: invalid auth method");
      }
...
class SocialAuthProvider {
   ...
   */
  async refreshToken(token) {
    return telemetry_index.withSpan(telemetry_definitions_index.TelemetryNamespace.Auth, "social-provider.refreshToken", async (span) => {
      span.setAttribute("authProvider", token.provider);
   ...
   */
  async logout(token) {
    return telemetry_index.withSpan(telemetry_definitions_index.TelemetryNamespace.Auth, "social-provider.logout", async (span) => {
      span.setAttribute("authProvider", token.provider);
   ...
   */
  async deleteAccount(token) {
    return telemetry_index.withSpan(telemetry_definitions_index.TelemetryNamespace.Auth, "social-provider.deleteAccount", async (span) => {
      span.setAttribute("authProvider", token.provider);
      return this.authServiceClient.deleteAccount(token.accessToken);
    });
  }
...
class ExternalIdpAuthProvider {
   ...
   */
  async authenticate(options) {
    return telemetry_index.withSpan(telemetry_definitions_index.TelemetryNamespace.Auth, "external-idp-provider.authenticate", async (span) => {
      if (options.authMethod !== "external_idp") {
        throw new ssoOidcClient.UnexpectedIssueError("ExternalIdpAuth: invalid auth method");
      }
      ...
      errors.logger.debug("ExternalIdpAuth: Starting OIDC discovery for issuer: %s, clientId: %s", issuerUrl, clientId);
      const discoveryStart = Date.now();
      let config;
      try {
        config = await client__namespace.discovery(new URL(issuerUrl), clientId, void 0, client__namespace.None());
      ...
      if (!tokenEndpoint) {
        span.setAttribute("failurePhase", "external_idp_discovery");
        throw new ssoOidcClient.UnexpectedIssueError("ExternalIdpAuth: token_endpoint not found in discovery");
      }
      errors.logger.info(
      ...
      const codeVerifier = client__namespace.randomPKCECodeVerifier();
      const codeChallenge = await client__namespace.calculatePKCECodeChallenge(codeVerifier);
      const state = client__namespace.randomState();
      ...
      const authUrl = client__namespace.buildAuthorizationUrl(config, authUrlParams);
      errors.logger.debug("ExternalIdpAuth: Authorization URL built: %s", authUrl.href);
      errors.logger.info("ExternalIdpAuth: Opening browser for authentication");
      const browserOpenStart = Date.now();
      let opened;
      try {
        opened = await vscode__namespace.env.openExternal(vscode__namespace.Uri.parse(authUrl.href), { skipValidation: true });
      ...
      if (!opened) {
        this.currentAuthState = void 0;
        span.setAttribute("failurePhase", "external_idp_browser_open");
        throw new ssoOidcClient.UnexpectedIssueError("ExternalIdpAuth: Failed to open browser for authentication");
      }
      ...
      try {
        ...
        errors.logger.info("ExternalIdpAuth: Received authorization code");
      } catch (error) {
        span.setAttribute("callbackWaitDurationMs", Date.now() - callbackWaitStart);
      ...
      errors.logger.debug("ExternalIdpAuth: Token exchange - callbackUrl: %s", callbackUrl.href);
      const tokenExchangeStart = Date.now();
      let tokens;
      try {
        tokens = await client__namespace.authorizationCodeGrant(config, callbackUrl, {
        ...
        if (error instanceof client__namespace.ResponseBodyError) {
        ...
        } else if (error instanceof client__namespace.WWWAuthenticateChallengeError) {
        ...
        } else if (error instanceof client__namespace.AuthorizationResponseError) {
          ...
          errors.logger.error("ExternalIdpAuth: Token exchange failed (unknown): %s", error);
        }
        throw error;
      }
      errors.logger.info("ExternalIdpAuth: Token exchange successful");
      const token = {
   ...
   */
  async refreshToken(token) {
    return telemetry_index.withSpan(telemetry_definitions_index.TelemetryNamespace.Auth, "external-idp-provider.refreshToken", async (span) => {
      span.setAttribute("authProvider", token.provider);
      ...
      if (!issuerUrl) {
        ...
        throw new ssoOidcClient.UnexpectedIssueError("ExternalIdpAuth: missing issuerUrl in stored token");
      }
      let issuerUrlParsed;
      try {
        ...
        span.setAttribute("failurePhase", "external_idp_validation");
        throw new ssoOidcClient.UnexpectedIssueError(`ExternalIdpAuth: invalid issuerUrl in stored token: ${issuerUrl}`);
      }
      const discoveryStart = Date.now();
      let config;
      try {
        config = await client__namespace.discovery(issuerUrlParsed, clientId, void 0, client__namespace.None());
        errors.logger.info("ExternalIdpAuth: OIDC discovery completed in %dms", Date.now() - discoveryStart);
      } catch (error) {
        span.setAttribute("discoveryDurationMs", Date.now() - discoveryStart);
        span.setAttribute("failurePhase", "external_idp_discovery");
      ...
      try {
        ...
        const tokens = await client__namespace.refreshTokenGrant(config, refreshToken, refreshParams);
        ...
        if (error instanceof client__namespace.ResponseBodyError) {
          ...
          span.setAttribute("errorType", "unknown");
          errors.logger.error("ExternalIdpAuth: Refresh failed (unknown): %s", error);
        }
        throw error;
      }
    });
  ...
  deleteAccount(_token) {
    return Promise.reject(new ssoOidcClient.UnexpectedIssueError("Account deletion not supported for external IdP authentication"));
  }
...
exports.AUTH_CALLBACK_REDIRECT_URI = AUTH_CALLBACK_REDIRECT_URI;
exports.AuthCallbackHandler = AuthCallbackHandler;
exports.ClientRegistrationStorage = ClientRegistrationStorage;
exports.ExternalIdpAuthProvider = ExternalIdpAuthProvider;
exports.IDCAuthProvider = IDCAuthProvider;
exports.SocialAuthProvider = SocialAuthProvider;
exports.authCallbackHandler = authCallbackHandler;
exports.handleProfiles = handleProfiles;
exports.selectOrPromptProfile = selectOrPromptProfile;
exports.supportsProfiles = supportsProfiles;
