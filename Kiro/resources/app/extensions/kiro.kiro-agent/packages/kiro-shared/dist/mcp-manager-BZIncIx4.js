...
import { MetricReporter, T as TelemetryNamespace } from "./telemetry/definitions/index.js";
import { Sema } from "async-sema";
import { c as mcpLogger, m as mapUnknownToErrorType } from "./errors-BVoUlRsM.js";
...
import { g as getActiveMcpConfigLocation, a as getHomeKiroPath } from "./paths-D3AmlBbI.js";
import { z } from "zod";
import { c as RegistryServerConfigEntrySchema } from "./resolve-registry-entries-DSUmtYNT.js";
import * as JSONC from "comment-json";
import { isJSONRPCRequest, isJSONRPCNotification, isJSONRPCErrorResponse, LATEST_PROTOCOL_VERSION, McpError, ErrorCode } from "@modelcontextprotocol/sdk/types.js";
...
function loadMcpConfig() {
  ...
  if (powerServers.length > 0) {
    mcpLogger.debug(`[Powers Debug] Loaded ${powerServers.length} power MCP servers: ${powerServers.join(", ")}`);
  }
...
function expandEnvironmentVariables(value, options) {
  if (typeof value === "string") {
    ...
    const approvedVars = expandAll ? void 0 : vscode.workspace.getConfiguration("kiroAgent").get(APPROVED_ENV_VARS_KEY, []);
...
async function surfaceConfigError(errorMsg, sourceName, configPath) {
  ...
  if (selection === "Ask Kiro") {
    ...
    void vscode.commands.executeCommand("kiroAgent.focusChatInput", { prompt: message, clear: true });
    void vscode.commands.executeCommand("kiroAgent.userInputFocusNoSubmit", message);
...
function deepValidateMCPServerOptions(serverName, options) {
  if (options.type === "registry") {
    const result = RegistryServerConfigEntrySchema.safeParse(options);
    if (!result.success) {
      return {
        isValid: false,
...
function resetApprovedEnvVars() {
  void vscode.workspace.getConfiguration("kiroAgent").update(APPROVED_ENV_VARS_KEY, [], vscode.ConfigurationTarget.Global);
  hasShownWarningThisSession = false;
...
class InstrumentedTransport {
  ...
  async send(message, options) {
    ...
    try {
      ...
      this.metrics.reportCountMetrics(
        { sendFailed: 1 },
        { ...base, Activity: activity, errorType: mapUnknownToErrorType(e) }
      );
      throw e;
    }
  }
  ...
  set onmessage(cb) {
    ...
    this.inner.onmessage = cb ? (m, extra) => {
      ...
      } else if (isJSONRPCNotification(m)) {
  ...
  set onerror(cb) {
    ...
    this.inner.onerror = cb ? (err) => {
      this.metrics.reportCountMetrics(
        { connectionError: 1 },
        { ...this.dims(), errorType: mapUnknownToErrorType(err) }
      );
      cb(err);
    } : void 0;
  }
...
const mcpTransportMetrics = new MetricReporter(TelemetryNamespace.Feature, "mcp");
...
class MCPConnection {
  ...
  async connectClient(signal, fallbackToSSE = false) {
    ...
    try {
      ...
      mcpLogger.info(`Connected to server with transport: ${transportLabel}`, this.serverName);
      this.recordInitialized(transport, transportLabel);
    } catch (error) {
      const shouldFallbackToSSE = !fallbackToSSE && error instanceof StreamableHTTPError && error.code && error.code >= 400 && error.code < 500;
      if (shouldFallbackToSSE) {
        this.connectPromise = null;
        await this.connectClient(signal, true);
      } else if (error instanceof UnauthorizedError && this.oauthRedirectServer && // eslint-disable-next-line @typescript-eslint/no-deprecated
      (rawTransport instanceof StreamableHTTPClientTransport || rawTransport instanceof SSEClientTransport)) {
  ...
  async doReAuthenticate() {
    ...
    try {
      ...
      const result = await auth(this.oauthProvider, { serverUrl });
      if (result === "REDIRECT") {
      ...
      mcpLogger.info("Mid-session re-authentication completed successfully", this.serverName);
    } finally {
    ...
    try {
      ...
      if (capabilities?.tools) {
        try {
          ...
          if (e instanceof McpError && e.code === ErrorCode.MethodNotFound.valueOf()) {
            ...
            mcpLogger.error(`Error listing tools: ${errorMsg}`, serverName);
          }
        }
      }
      if (capabilities?.resources) {
        try {
          ...
          if (e instanceof McpError && e.code === ErrorCode.MethodNotFound.valueOf()) {
            ...
            const errorMsg = e instanceof Error ? e.message : String(e);
            mcpLogger.error(`Error listing resources: ${errorMsg}`, serverName);
          }
        }
        try {
          ...
          if (e instanceof McpError && e.code === ErrorCode.MethodNotFound.valueOf()) {
            mcpLogger.warn("MCP server does not support listing resource templates. Skipping...", serverName);
          } else {
            const errorMsg = e instanceof Error ? e.message : String(e);
            mcpLogger.error(`Error listing resource templates: ${errorMsg}`, serverName);
          }
        }
      }
      if (capabilities?.prompts) {
        try {
          ...
          if (e instanceof McpError && e.code === ErrorCode.MethodNotFound.valueOf()) {
            mcpLogger.warn("MCP server does not support listing prompts. Skipping...", serverName);
          } else {
            const errorMsg = e instanceof Error ? e.message : String(e);
            mcpLogger.error(`Error listing prompts: ${errorMsg}`, serverName);
          }
        }
      }
      return {
      ...
      mcpLogger.error(`Error syncing MCP server: ${errorMsg}`, serverName);
      throw error;
    } finally {
      if (this.oauthRedirectServer) {
...
class MCPManagerSingleton {
  ...
  constructor() {
    ...
    this.connectionSemaphore = new Sema(MCP_MAX_CONCURRENT_CONNECTIONS);
  }
  /**
  ...
  async startConnection(serverName, options, configuredIndex, useOAuth, onConnectionChange, cwd) {
    ...
    try {
      ...
      if (allConnectionsSettled && !this.hasShownToolCountWarning && numTools > MCP_TOOL_COUNT_WARN_THESHOLD) {
        ...
        `;
        void (async () => {
          const action = await vscode.window.showWarningMessage(warnMessage, "Show MCP Servers");
          if (action === "Show MCP Servers") {
            await vscode.commands.executeCommand("kiroAgent.views.mcpServerStatus.focus");
          }
        })();
...
export {
  MCPConnection as M,
  OAuthConfigSchema as O,
  MCPJsonConfigSchema as a,
  MCPManagerSingleton as b,
  MCPOptionsSchema as c,
  addMCPServerConfig as d,
  addMCPToolToAutoApproveConfig as e,
  clientCapabilityEvents as f,
  deepValidateMCPServerOptions as g,
  disableMCPTools as h,
  enableMCPTools as i,
  expandEnvironmentVariables as j,
  findConfigFileForServer as k,
  formatToolName as l,
  isHttpsOrLocalhost as m,
  loadMcpConfig as n,
  mcpServerSources as o,
  parseRedirectPort as p,
  setMCPServerDisabled as q,
  resetApprovedEnvVars as r,
  serverCapabilityEvents as s
};
